# Tech Companies Stacks and Practices

| Company | Product or Platform | Key Engineering Challenges | Engineering Culture and Values | Internal Tools and Tech Stack | Performance Management System | Compensation and Hiring Model | AI and LLM Integration |
| :---- | :---- | :---- | :---- | :---- | :---- | :---- | :---- |
...
| Term | Simplified Definition | Video where first mentioned | Context of Use |
| :---- | :---- | :---- | :---- |
| RAG | A method to enhance AI models by connecting them to specific, external data sources to provide more accurate or context-aware answers. | 034 Amazon, Google and Vibe Coding with Steve Yegge | Discussed as a technique where a graph database can be used in a "RAG-like" style to help AI understand legacy codebases, and as the "hello world" of building LLM software for internal documentation. |
| Vibe Coding | A development style where a developer uses AI agents to generate code quickly based on high-level prompts or "vibes" rather than manual line-by-line coding. | 034 Amazon, Google and Vibe Coding with Steve Yegge | Described as an addictive mode where AI does the work, though experts warn it can break the learning loop or lead to recklessness if shortcuts aren't understood. |
| Microservices | An architectural style that structures an application as a collection of small, independent services that communicate with each other. | 034 Amazon, Google and Vibe Coding with Steve Yegge | Mentioned as a pattern that can lead to debates during technical reviews and the structure that necessitated the creation of Kubernetes. |
| Borg | Google's internal system for managing large-scale clusters of computers and scheduling jobs across them. | 034 Amazon, Google and Vibe Coding with Steve Yegge | Used as an example of an internal tool that scales significantly better than public alternatives and served as the predecessor to Kubernetes. |
| Spec-driven development | A methodology where developers provide a highly detailed specification or blueprint to an AI or team before any code is written. | 044 How AI will change software engineering – with Martin Fowler | Discussed as an experiment to see if better specifications help AI produce more reliable software, forcing a return to fundamental practices. |
| Piper | Google's custom-built version control system used to manage their massive unified code repository (monorepo). | 040 Google’s engineering culture | Identified as the internal alternative to GitHub for managing Google's billions of lines of code. |
| Open Connect | Netflix's custom-built content delivery network (CDN) that places servers in data centers globally to reduce streaming latency. | 043 Netflix’s Engineering Culture | Described as a "superpower" and strategic advantage for Netflix that allows for high-quality streaming at a global scale. |
| MCP (Model Context Protocol) | An open standard that allows AI models to connect to different data sources and tools, such as databases, local files, or web browsers. | 034 Amazon, Google and Vibe Coding with Steve Yegge | Steve Yegge describes using an MCP server to connect his database to an AI agent; Shopify uses it to give AI access to internal wikis. |
| Monorepo | A software development strategy where all of a company's code is stored in a single, massive repository instead of multiple smaller ones. | 040 Google’s engineering culture | Discussed as a core part of the engineering culture at Google, Meta, Uber, and Reddit to manage dependencies and standardized updates. |
| Stubby | Google's internal framework for Remote Procedure Calls (RPC) that allows services to communicate with each other. | 034 Amazon, Google and Vibe Coding with Steve Yegge | Mentioned as the high-performance internal predecessor to the open-source gRPC. |
| DRI (Directly Responsible Individual) | A management concept where one specific person is assigned clear ownership and accountability for a project's success. | 002 Efficient scaleups in 2024 vs 2021: Sourcegraph | Described as the role necessary for a project to successfully ship in large organizations like Sourcegraph. |
| RFC (Request for Comments) | A technical document used to propose and discuss new features or architectural changes within an engineering team. | 038 High growth startups: Uber and CloudKitchens | Used as a medium for technical debates and a way for senior engineers to influence project direction at companies like Notion and Uber. |
| Spanner | A globally distributed database at Google that provides strong consistency for data across multiple data centers. | 040 Google’s engineering culture | Described as an internal database used when global consistency is more important than speed. |
| Blaze | Google's internal tool for building and testing software, designed to be extremely fast for large projects. | 040 Google’s engineering culture | Cited as the tool that enables Google's monorepo to function; later open-sourced as Bazel. |
| Critique | Google's internal code review tool used by engineers to provide feedback and approve changes. | 040 Google’s engineering culture | Mentioned as a central part of the developer workflow, integrated with the IDE and test systems. |
| Chubby | A highly reliable distributed locking service used at Google for managing shared resources across clusters. | 034 Amazon, Google and Vibe Coding with Steve Yegge | Cited as an example of an engineering problem Google solved with "seven nines" of availability. |
| Toil | Routine, manual, and repetitive tasks related to operating a service that provide no enduring value and scale linearly. | 040 Google’s engineering culture | Discussed in the context of Google SRE teams having "Toil SLOs" to ensure engineers aren't overwhelmed by manual work. |
| Keeper Test | A management practice at Netflix where leaders ask if they would fight to keep an employee if they resigned. | 043 Netflix’s Engineering Culture | Used to explain how Netflix maintains high talent density without traditional performance reviews. |
| Mojo | A new programming language designed for AI development that combines Python syntax with high-performance hardware control. | 042 From Swift to Mojo and high-performance AI Engineering | Presented as a solution to unify high-level productivity with low-level kernel performance in AI engineering. |
| LLVM | A modular collection of compiler and toolchain technologies used to build programming languages like Swift, Rust, and C++. | 042 From Swift to Mojo and high-performance AI Engineering | Chris Lattner details the history of creating LLVM as a research project that eventually powered Apple's developer tools. |
| TDD (Test-Driven Development) | A practice where a programmer writes an automated test for a feature before writing the actual implementation code. | 029 TDD, AI agents and coding with Kent Beck | Kent Beck explains the origin of TDD and how it fits into modern workflows with AI tools. |
| DORA Metrics | A set of four key metrics used to measure software delivery performance: Deployment Frequency, Lead Time, Change Failure Rate, and Time to Restore. | 015 Developer productivity with Nicole Forsgren | Nicole Forsgren explains these as a signal for how well a development pipeline is working from commit to production. |
| SPACE Framework | A holistic framework for measuring developer productivity across five dimensions: Satisfaction, Performance, Activity, Communication, and Efficiency. | 015 Developer productivity with Nicole Forsgren | Introduced to move beyond simple output counts and capture the "lived experience" of developers. |
| StatSig | A tool (originally internal to Meta) that helps developers manage feature flags and run experiments to see how code affects users. | 010 The Coding Machine at Meta – with Michael Novati | Identified as a company seeded by Meta's internal experimentation platform "Gatekeeper." |
| Correction of Errors (COE) | Amazon's internal name for a post-mortem document used to identify and fix the root cause of a system failure. | 033 What is a Principal Engineer at Amazon? With Steve Huynh | Steve describes these as a "stream of disasters" that provide the best internal engineering reading and learning. |
| Six Pager | A detailed six-page narrative document used for proposing projects or promotions at Amazon. | 022 Working at Amazon as a software engineer – with Dave Anderson | Used to describe Amazon's culture of writing documents for everything, reflecting their document-heavy culture. |
| Bar Raiser | An experienced interviewer at Amazon from a different team who ensures high hiring standards and can veto a decision. | 022 Working at Amazon as a software engineer – with Dave Anderson | Discussed as a unique part of the Amazon interview process to ensure the hiring loop is run correctly. |
| Two Pizza Team | An organizational pattern where teams are kept small enough to be fed by two pizzas (typically 6–10 people). | 022 Working at Amazon as a software engineer – with Dave Anderson | A Level 6 manager usually manages a "two pizza team" where they own their specific space. |
| Coding Machine | An engineering archetype for an individual who produces an exceptionally high volume of impactful code, often single-handedly. | 010 The Coding Machine at Meta – with Michael Novati | The term was specifically invented at Meta to describe Michael Novati's unique role and high output. |
| Stacked Diffs | A workflow where a developer breaks a large feature into several small, dependent code changes reviewed and merged one by one. | 020 Stacked diffs and tooling at Meta with Tomas Reimers | Described as the "defacto way" at Meta to avoid being blocked by code reviews. |
| Enge Crit | A collaborative engineering review process at Figma using FigJam where engineers provide feedback via sticky notes. | 019 Building Figma Slides with Noah Finer and Jonathan Kaufman | Described as a 30-minute session where engineers leave "spicy" feedback on technical designs. |
| DevPods | A cloud-based tool that provides a pre-configured and ready-to-use development environment for engineers. | 018 Developer Experience at Uber with Gautam Korlam | Uber developed these to allow developers to boot a full environment with pre-indexed code in seconds. |
| Submit Queue | A system that ensures code changes don't break the main codebase by testing them in a specific order before merging. | 018 Developer Experience at Uber with Gautam Korlam | Uber's in-house tool used to guarantee a "green main" branch despite thousands of engineers committing. |
| Local first | A software design approach where data is stored and processed on the user's device first, then synced to the cloud. | 017 Design-first software engineering: Craft | Discussed in the context of personal software like Craft to ensure users can work without an internet connection. |
| SAST | A security tool that analyzes source code while it is stationary (not running) to find potential vulnerabilities. | 045 Code security for software engineers | Johannes discusses tools that transform a codebase into an abstract model to simulate runtime issues. |
| DAST | A security tool that tests a running application from the outside, often by simulating hacker attacks. | 045 Code security for software engineers | Explained as blackbox testing from the outside while the application is running in production. |
| SCA | A tool that scans a project to identify third-party libraries and open-source components, checking them for security risks. | 045 Code security for software engineers | Mentioned as an advanced tool that checks manifest files against databases of known threats like CVEs. |
| Kernel | The core part of an operating system that manages the computer's hardware and provides basic services for other programs. | 032 How Linux is built with Greg Kroah-Hartman | Discussed as the 5% of Linux code that manages memory, CPU, and hardware agnostically for user space. |
| Cardinality | In data science, the number of unique values in a specific set (e.g., User ID is high cardinality, Gender is low). | 011 Observability: the present and future, with Charity Majors | Described as the reason observability bills explode, as traditional tools struggle with high numbers of unique IDs. |
| SIMD | A type of parallel processing that allows a computer to perform the same operation on multiple data points simultaneously. | 042 From Swift to Mojo and high-performance AI Engineering | Described as essential for getting high performance out of modern CPUs, particularly in AI workloads. |
| Metaprogramming | A programming technique in which computer programs have the ability to treat other programs as their data. | 042 From Swift to Mojo and high-performance AI Engineering | Highlighted as a core feature of the Mojo language to allow developers to build compile-time transformations. |
| Zero Bug Policy | A principle where all reported bugs must be fixed immediately, typically within a day, rather than sitting in a backlog. | 005 Linear: move fast with little process | Linear uses this to prioritize current customer quality over building new features that haven't been seen yet. |
| Trial Week | A hiring stage where a candidate works full-time with the company for a week on a real project before being hired. | 005 Linear: move fast with little process | Described as Linear's famously different hiring process where candidates work autonomously on Greenfield projects. |
| Scuba | A fast, in-memory data store used internally at Facebook for real-time slicing and dicing of telemetry data. | 011 Observability: the present and future, with Charity Majors | Charity Majors describes it as a "weird beast" that allowed Facebook to pinpoint MySQL issues in seconds. |
| Readability Review | A process at Google where code must be approved by someone certified as an expert in writing clear, standard-compliant code. | 004 Promotions and tooling at Google | Irina describes the process of getting certified to ensure code is intuitive for others at Google. |
| ZIRP | An economic environment with near-zero interest rates that previously fueled massive venture capital spending in tech. | 038 High growth startups: Uber and CloudKitchens | Used to explain why the era of hypergrowth and massive hiring at startups like Uber has largely ended. |
| Founding Engineer | The first engineer(s) to join a startup, responsible for building the initial product and established technical foundation. | 046 Being a founding engineer at an AI startup | Michelle Lim's role at Warp and a central topic of her career advice regarding startup technical foundations. |
| Game Day | A simulation or drill where a team tests their systems under production-like stress and traffic before a major event. | 014 Live streaming at world-record scale | Used at Jio Cinema to prepare for the massive traffic of the IPL cricket tournament. |
| SRE | Site Reliability Engineering; a discipline that applies software engineering to system administration and operations. | 016 The man behind the Big Tech comics – with Manu Cornet | Represented in Manu Cornet's comics as the person making the infrastructure stable for a project. |
| KEP | Kubernetes Enhancement Proposal; a formal process for proposing and documenting changes to the Kubernetes project. | 026 How Kubernetes is Built with Kat Cosgrove | Described as a mandatory requirement for including new features in a Kubernetes release. |
| HLS / DASH | Standard technical protocols used to deliver video over the internet in small chunks or segments. | 014 Live streaming at world-record scale | Explained as the "secret sauce" of how live streaming works across different devices. |
| Prompt Injection | A security vulnerability where an attacker crafts specific inputs to trick an AI into ignoring its instructions. | 045 Code security for software engineers | Johannes describes prompt injection as "the new code injection" where human language is used to mess with LLM logic. |
| Speculative decoding | An optimization technique where a smaller model predicts tokens to speed up the inference process of a larger model. | 025 Building Windsurf with Varun Mohan | Discussed as an optimization technique used in the Windsurf inference stack to reduce latency. |
| Fill in the middle | A specific AI coding capability where the model predicts code based on both the preceding and following context. | 025 Building Windsurf with Varun Mohan | Varun Mohan describes this as a "table stakes" capability for coding that standard text models often lack. |
